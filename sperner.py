...
PARSER.add_argument(
    ...
    action="store_true",
    required=False,
    help="dont show graph (if relevant)",
)
PARSER.add_argument(
    "--save",
    type=str,
...
if dim <= 2:  # geometic realization is plottable
    ...
    quite_simplex.plot_silly_graph(
        quite_simplex.make_silly_graph(crossing_colors=crossing_colors),
    ...
    quite_simplex.plot_rainbow_faces(
        dim2kwargs=dict(
            color="purple",
            alpha=0.5,
            zorder=-1,
    ...
    if args.save is not None:
        plt.savefig(args.save, bbox_inches="tight", dpi=args.dpi)
