...
class SimplexSubdivision:
    ...
    def make_silly_graph(self, crossing_colors=None):
        ...
        for face in self.faces_by_dim[-1]:
            if self.track_adjacency:
                ...
                neigh = [
                    (facep, set.intersection(set(face), set(facep)))
    ...
    def plot_coloring(
        ...
        E = [[self.V[i], self.V[j]] for i, j in self.faces_by_dim[1]]  # 1 dim faces
        for p, q in E:
            plotter.plot(
                [p[0], q[0]],
                [p[1], q[1]],
    ...
    def plot_silly_graph(
        ...
        if inf_vertex_pos is None:
            if self.dim == 1:
                inf_vertex_pos = np.array([1.25, -0.25])
            if self.dim == 2:
        ...
        if show_pts:
            plt.scatter(
                [hi_dim_face_pts[k][0] for k in hi_dim_face_pts],
